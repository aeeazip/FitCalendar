...
public class MatchingDao {
	...
	public Exerciser showOption(int exerciserId) {
		...
		try {
			...
			if (rs.next()) {
				exerciser = new Exerciser(rs.getInt("exerciserId"), rs.getString("useMatchSvc"), rs.getInt("maxMate"));
			}
			return exerciser;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<MatchingStatus> showSitationList(int exerciserId) {
		...
		try {
			...
			while (rs.next()) {
				MatchingStatus matchingStatus = new MatchingStatus(rs.getInt("senderId"), rs.getInt("receiverId"),
						rs.getInt("status"));
